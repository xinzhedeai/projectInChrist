/*操控左侧手风琴菜单*/
	$(document).ready(function() {
		...
		getLeftDropListNo();//获取左侧下拉手风琴的数据
...
function getLeftDropListNo(){//获取用户详情
	$.post('/userAction/getLeftDropListNo.action', function(res) {
		if (res.success) {
			var rows = res.result 
		}
			if(rows!=null){
				$(".inChristWeb").text(rows.inChristTotal);
				$(".gospel").text(rows.gospelNo);
				$(".music").text(rows.musicNo);
				$(".wanted").text(rows.wantedNo);
				$(".shop").text(rows.shopNo);
				$(".friend").text(rows.friendNo);
				$(".webFamily").text(rows.webFamilyNo);
