$(function(){
	...
	$('#fSAllQtDg').datagrid({
			url : '/fellowShipAction/searchAllQt.action?fsId='+fellowShipId,
			idField : 'QUESITION_ID',
			pagination : true,
			fitColumns : true,
			rownumbers : true,
			checkOnSelect:false,
			...
			columns : [[ {
				...
				formatter : function(value, row, index) { 
					var a = '<a href="fSQuesitionDetail.html?quesitionId='+row.QUESITION_ID+'">'+value+'</a>';
					console.info(row.QUESITION_ID);
					return a;
				}
			}, {
				...
				width : 220,
				formatter : function(value, row, index) { 
					var a = '<span>'+row.QUESITION_REPLY_NO+'/'+row.VIEWS+'</span>';
					return a;
				}
			}, {
				...
				formatter : function(value, row, index) { 
					if(value  == 'Y'){
						value ='<i class="fa fa-lg  fa-check"></i>';
					}else{
						value ='<i class="fa fa-lg  fa-times"></i>';
					}
			...
			loadFilter : function(data) {
				if (data != null) {
					...
					if(data.result.rows != null){
						...
						$('#fSAllQtDg').datagrid('loadData', { total: 0, rows: [] });
					}
				} else {
    ...
    var flag = 0;
    $("#noSoluted").click(function(){
    	 console.info("未解决tab事件触发。");
		 if(flag == 0){
		    	console.info(flag);
				$('#fSNoSolutedQtDg').datagrid({
					url : '/fellowShipAction/searchNoSolutedQt.action?fsId='+fellowShipId,
					idField : 'QUESITION_ID',
					pagination : true,
					fitColumns : true,
					rownumbers : true,
					...
					loadFilter : function(data) {
						if (data != null) {
							...
							if(data.result.rows != null){
								...
								var txt=  "没有相关数据！";
								window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
								$('#fSNoSolutedQtDg').datagrid('loadData', { total: 0, rows: [] });
							}
						} else {
							window.location.href='/admin/login.html';
						}
    ...
    var flag1 = 2;
    $("#soluted").click(function(){
    	 console.info("未解决tab事件触发。");
		 if(flag1 == 2){
				$('#fSSolutedQtDg').datagrid({
					url : '/fellowShipAction/searchSolutedQt.action?fsId='+fellowShipId,
					idField : 'QUESITION_ID',
					pagination : true,
					fitColumns : true,
					rownumbers : true,
					...
					loadFilter : function(data) {
						if (data != null) {
							if(data.result.rows != null){
								...
								var txt=  "没有相关数据！";
								window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
								$('#fSSolutedQtDg').datagrid('loadData', { total: 0, rows: [] });
							}
						} else {
							window.location.href='/admin/login.html';
						}
	...
	$("#addQBtn").click(function(){//编辑器保存按钮
		var title = $(".fsQTitle").val();
		var content = UE.getEditor('logEditor').getContent();
		addFSQ(fellowShipId, title, content);
...
function addFSQ(fellowShipId,title, content){
	$.post('/fellowShipAction/addFSQ.action', $.param({'usId':loginUsId,'fsId':fellowShipId,'fsQTitle':title,'fsQContent':content}), function(res) {
		if (res.success) {
