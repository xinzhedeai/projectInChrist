...
$(document).ready(function() {
	$(document).ready(function() {
		...
		$video.pageA = 1,rows = 12;//pageA指的是请求后台数据分页的page
		var flag = null;
	
		page=1;//初始化当前的版面为1
		$show=$("#zzsc").find(".zzsc_box");//找到图片展示区域
		
		getVideoPages($video.pageA,rows);//初始页面请求接口
		
		$(".next").click(function(){
			$video.pageA += 1;
			$video.numsA = $video.pageA * rows;
			if($video.flagA == 1){//当设置全局标志位为数字时，初始值尽量不要是null，最好是1还是其他数字，否则在做判断是会有问题。
				getVideoPages($video.pageA,rows);
			}
			//首先判断展示区域是否处于动画
		//	if(!$show.is(":animated")){
				$show.animate({left:'-='+$width_box},"normal");
				page++;
				nav(page,$video.page_count);
				$number=page-1;
	...
	function getVideoPages(pageA,rows){//获取主内工作网站
		$.post('/userAction/getJobPages.action', $.param({'page':pageA,'rows':rows,'webType':'A01'}), function(res) {
			var rows = null;
			if (res.success) {
				rows = res.result.rows ;
				if(rows){
					$video.total = res.result.total;
					var pageNum = Math.ceil($video.total/12);
					...
					var p = '';//中文p
						...
						$(".zzsc_box").append(p);
						if($video.numsA > $video.total){
							$video.flagA = 0;
						}
						$video.page_count = pageNum;
						nav(page,$video.page_count );
