...
$(document).ready(function() {
	$(document).ready(function() {
		...
		$shop.pageA = 1,rows = 12;//pageA指的是请求后台数据分页的page
		var flag = null;
	
		page=1;//初始化当前的版面为1
		$show=$("#zzsc").find(".zzsc_box");//找到图片展示区域
		
		getShopPages($shop.pageA,rows);//初始页面请求接口
		
		$(".next").click(function(){
			$shop.pageA += 1;
			$shop.numsA = $shop.pageA * rows;
			if($shop.flagA == 1){//当设置全局标志位为数字时，初始值尽量不要是null，最好是1还是其他数字，否则在做判断是会有问题。
				getShopPages($shop.pageA,rows);
			}
			//首先判断展示区域是否处于动画
		//	if(!$show.is(":animated")){
				$show.animate({left:'-='+$width_box},"normal");
				page++;
				nav(page,$shop.page_count);
				$number=page-1;
	...
	function getShopPages(pageA,rows){//获取主内工作网站
		$.post('/userAction/getJobPages.action', $.param({'page':pageA,'rows':rows,'webType':'A04'}), function(res) {
			var rows = null;
			if (res.success) {
				rows = res.result.rows ;
				if(rows){
					$shop.total = res.result.total;
					var pageNum = Math.ceil($shop.total/12);
					...
					var p = '';//中文p
						...
						$(".zzsc_box").append(p);
						if($shop.numsA > $shop.total){
							$shop.flagA = 0;
						}
						$shop.page_count = pageNum;
						nav(page,$shop.page_count );
