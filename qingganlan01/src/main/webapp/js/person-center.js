...
//$("#search").click(function(){//搜索
	...
	$(".addFriend").click(function(){
		//这里的name是通过自动选择框里面的用户id。
		var friendId = $("#dialogInput").attr("name");
		console.info(friendId);
		addFriend(loginUsId,friendId,paramUserId);
	})
	$(".showFriendModal").click(function(){
		getUsers();
	})
	$('.fsPsContent').keyup(autoWriteContent);
	var uploaders = new plupload.Uploader({//发表图片说说上传图片。
	    browse_button : 'uploadFSHeadImg', // you can pass in id...
	    url : "/fellowShipAction/uploadFSHeadImg.action",
	    filters : {
	     //   max_file_size : '3mb',
	        mime_types: [
...
//		    resize : {width:120, height:120, quality:90, crop:false, preserve_headers:true},
				...
				if(res.result && res.result.files) {
				  //$(".l-setting").css("height",977)
					var file = res.result.files[0];
					 ...
					 fellowShipHeadImg = file.FILE_ID + '.' +  file.FILE_EXTENSION;
					
					 //这里的MODU_CD是保存图片的时候传给后台的。
					 upfile1 = '[{"MODU_CD":"/fellowShip","file_ids":["' + file.FILE_ID + '"]}]';
					 console.info(fellowShipHeadImg);
	...
	searchMyFris(loginUsId,paramUserId,page,rows);
	searchMyFellowShips(loginUsId,page,rows,paramUserId);
	searchMyJoinFS(loginUsId,page,rows)
	$(".addFellowShip").click(function(){
		addFellowShip(fellowShipHeadImg,paramUserId);
	});
})
...
function getMyBlogs(paramUserId,page,rows){//获取所有自己和好友说说（这个数据库查询语句待修改。）
	$.post('/blogAction/searchMyBlogs.action',$.param({"usId":paramUserId,"page":page,"rows":rows}), function(res) {
		if (res.result != null) {
...
function deleteBlog(blogId,loginUsId){
	 $.post('/blogAction/deleteBlog.action', $.param({'blogId':blogId}), function(result) {
		    if(result.success){
		    	var txt= result.msg;
				window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
				location.href = 'person-center.html?paramUserId='+loginUsId;
...
function searchMyFellowShips(loginUsId,page,rows,paramUserId){//我创建的团契
	 $.post('/fellowShipAction/searchMyFellowShips.action', $.param({'usId':loginUsId,'page':page,'rows':rows}), function(res) {
		    if(res.result){
		    	...
		    	for(var i=0;i<rows.length;i++){
		    		p += '<li class="myFriendsLi">';
		    		p += '<a href="fellowShip.html?fellowShipId='+rows[i].fsId+'"><img src="/storage/upload/fellowShip/'+rows[i].fsHeadImg+'">'+rows[i].fsName+'</a>';
		    		p += '<i class="fa fa-md fa-sign-out" onclick="confirmdelFellow(\''+rows[i].fsId+'\',\''+paramUserId+'\');"></i></li>';
		    	}
...
function getUsers(){
	 $.post('/userAction/getAllUsers.action', $.param({}), function(res) {
		    if(res.result){
		    	var rows = res.result.rows;
		    	console.info(rows);
		    	if(rows){
					...
					//NodeID//Nodetitle
				      console.info(this.id + "," + this.innerHTML);
				      $("#dialogInput").attr("name",this.id);
				     // console.info(rows);
				    };
				    $.selectSuggest('dialogInput',rows,itemSelectFuntion);
				}
			}else{
				var txt= "返回数据为空！";
				window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
...
function addFellowShip(fSHeadImg,paramUserId){//创建团契
	 $.post('/fellowShipAction/addFellowShip.action', $.param({'fSHeadImg':fSHeadImg,'fSBuildUsId':loginUsId})+ '&' +$('#newFellowForm').serialize(), function(res) {
		    if(res.success){
	    		var txt= "添加团契成功！";
				window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
				location.href = "person-center.html?paramUserId="+paramUserId;
...
function deleteFellow(fsId,paramUserId){//删除自己创建的团契
...
function confirmdelFellow(fsId,paramUserId){//提示框，是否删除。
	var txt=  "该操作会删除该团契相关的所有信息!!请谨慎操作!";
	var option = {
			title: "系统信息",
			btn: parseInt("0011",2),
			onOk: function(){
				deleteFellow(fsId,paramUserId);
			},
		onCancel: function(){
...
function autoWriteContent(){
    var fSPSContent=$(this).val().substr(0,80);
    $(this).val(fSPSContent);
    $(".textAreaCount").text($(this).val().length+"/80");
};