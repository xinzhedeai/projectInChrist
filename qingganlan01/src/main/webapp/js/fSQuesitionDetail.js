		...
		if ( result!= null) {
			...
			if(rows != null){
				for(var i=0;i<rows.length;i++){
					...
					var solutedStyle = "block";
					var noSolutedStyle = "none";
					if(rows[i].IS_OK == 'N'){
						solutedStyle = "none";
						noSolutedStyle = "block";
					}
					p += '<i  class="fa fa-lg  fa-thumbs-up  noSoluted'+rows[i].REQUEST_REPLY_ID+'" style="display:' + noSolutedStyle + ';" onclick="confirmSoluted(\''+rows[i].REQUEST_REPLY_ID+'\');"></i>';
					p += '<i  class="fa fa-lg fa-angellist soluted'+rows[i].REQUEST_REPLY_ID+'" style="display:' + solutedStyle + ';"></i>';
...
function isSoluted(requestReplyId,quesitionId){
	$.post('/fellowShipAction/isSoluted.action', $.param({'requestReplyId':requestReplyId,'quesitionId':quesitionId}), function(res) {
		if (res.success) {
			$(".noSoluted" + requestReplyId).hide();
			$(".soluted" + requestReplyId).show();
