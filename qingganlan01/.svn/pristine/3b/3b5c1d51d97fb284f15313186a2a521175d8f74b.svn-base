	 ...
	 $.post('/userAction/searchMyConcern.action', $.param({'usId':paramUserId,'page':page,'rows':rows}), function(res) {
		    if(res.result){
		    	...
		    	for(var i=0;i<rows.length;i++){
		    		...
		    		var q = '';//之前把这个变量放到外面后出现了地址重复显示的情况，原因是公用一个变量，值累加。
		    		...
		    		if(paramUserId == loginUsId){
		    			p += '<i class="fa fa-lg fa-trash-o concernTrash"  onclick="confirmDelete(\''+rows[i].US_ID+'\');" ></i>';
		    		}
		    		p += '</div></li>';
...
function cancelConcern(concernedUsId){
	 $.post('/userAction/delMyConcern.action', $.param({'concernedUsId':concernedUsId,'usId':loginUsId}), function(result) {
		    if(result.success){
		    	/*var txt= result.msg;
				window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);*/
		    	var txt=  result.msg;
		    	var option = {
		    			...
		    			btn: parseInt("0001",2),
		    			onOk: function(){
		    				location.href = 'myConcern.html?paramUserId='+loginUsId;
		    			},
		    		onCancel: function(){
...
function confirmDelete(concernedUsId){//提示框，是否删除。
	var txt=  "确定取消对该用户的关注吗？";
	var option = {
			title: "系统信息",
			btn: parseInt("0011",2),
			onOk: function(){
				cancelConcern(concernedUsId);
			},
		onCancel: function(){
