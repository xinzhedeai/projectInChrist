...
$(document).ready(function() {
	$(document).ready(function() {
		...
		$music.pageA = 1,rows = 12;//pageA指的是请求后台数据分页的page
		var flag = null;
	
		page=1;//初始化当前的版面为1
		$show=$("#zzsc").find(".zzsc_box");//找到图片展示区域
		
		getMusicPages($music.pageA,rows);//初始页面请求接口
		
		$(".next").click(function(){
			$music.pageA += 1;
			$music.numsA = $music.pageA * rows;
			if($music.flagA == 1){//当设置全局标志位为数字时，初始值尽量不要是null，最好是1还是其他数字，否则在做判断是会有问题。
				getMusicPages($music.pageA,rows);
			}
			//首先判断展示区域是否处于动画
		//	if(!$show.is(":animated")){
				$show.animate({left:'-='+$width_box},"normal");
				page++;
				nav(page,$music.page_count);
				$number=page-1;
	...
	function getMusicPages(pageA,rows){//获取主内工作网站
		$.post('/userAction/getJobPages.action', $.param({'page':pageA,'rows':rows,'webType':'A02'}), function(res) {
			var rows = null;
			if (res.success) {
				rows = res.result.rows ;
				if(rows){
					$music.total = res.result.total;
					var pageNum = Math.ceil($music.total/12);
					...
					var p = '';//中文p
						...
						$(".zzsc_box").append(p);
						if($music.numsA > $music.total){
							$music.flagA = 0;
						}
						$music.page_count = pageNum;
						nav(page,$music.page_count );
