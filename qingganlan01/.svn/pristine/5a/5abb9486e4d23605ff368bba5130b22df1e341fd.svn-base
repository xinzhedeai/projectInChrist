...
$(document).ready(function() {
	$(document).ready(function() {
		...
		$summary.pageA = 1,rows = 12;//pageA指的是请求后台数据分页的page
		var flag = null;
	
		page=1;//初始化当前的版面为1
		$show=$("#zzsc").find(".zzsc_box");//找到图片展示区域
		
		getSummaryPages($summary.pageA,rows);//初始页面请求接口
		
		$(".next").click(function(){
			$summary.pageA += 1;
			$summary.numsA = $summary.pageA * rows;
			if($summary.flagA == 1){//当设置全局标志位为数字时，初始值尽量不要是null，最好是1还是其他数字，否则在做判断是会有问题。
				getSummaryPages($summary.pageA,rows);
			}
			//首先判断展示区域是否处于动画
		//	if(!$show.is(":animated")){
				$show.animate({left:'-='+$width_box},"normal");
				page++;
				nav(page,$summary.page_count);
				$number=page-1;
	...
	function getSummaryPages(pageA,rows){//获取综合网站
		$.post('/userAction/getJobPages.action', $.param({'page':pageA,'rows':rows,'webType':'A06'}), function(res) {
			var rows = null;
			if (res.success) {
				rows = res.result.rows ;
				if(rows){
					$summary.total = res.result.total;
					var pageNum = Math.ceil($summary.total/12);
					...
					var p = '';//中文p
						...
						$(".zzsc_box").append(p);
						if($summary.numsA > $summary.total){
							$summary.flagA = 0;
						}
						$summary.page_count = pageNum;
						nav(page,$summary.page_count );
