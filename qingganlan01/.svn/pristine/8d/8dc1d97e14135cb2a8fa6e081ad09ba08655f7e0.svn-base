...
$(function(){
	$(document).ready(function() {
		...
		/*$("#user-detail-btn").click(function() {
			...
			var rows=$('#az-user-tableList').datagrid("getSelections");
			if(rows.length!=1&&rows.length!=0){
			
				var txt = "只能查看一个用户的详细信息！";
				window.wxc.xcConfirm(txt,window.wxc.xcConfirm.typeEnum.info);
		...
		$("#modalDiv").on("click","#user-saveDetailModal-btn" ,function() {
			//不可修改的
			var userId=$("#detail_userId").text();
			
			//可修改的
			var patrn=/[`~!@#$%^&*_+<>?{}\/'[\]]/im; 
			var as=$("#detail_AbilityScope").val();
    		if(patrn.test(as)){  
    			var txt = "含有非法字符！";
				window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
				return false;
    		}
			var Limit = $("#detail_Limit").text();
			if(Limit == "受限"){
				Limit = "LIMIT";
			}else{
				Limit = "NORMAL";
			...
			var cu = $("#detail_ClassicUser").text();
			if(cu == "是"){
				cu = "Y";
			}else{
				cu = "N";
			}
			var pro=$("#detail_Profession").val();
			if(patrn.test(pro)){  
    			var txt=  "含有非法字符！";
				window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
				return false;
    		}
			$user.ModifyUser(userId, as, Limit, Level, cu, pro);
		});
			...
			$.post('/mUserAction/userDetail.action', $.param({'usId':usId}), function(res) {
				if (res.success) {
					...
					if (result) {
						...
						switch(Level){
						case "0":
							$(".L0").attr("selected", true);
							break;
						case "1":
							$(".L1").attr("selected", true);
							break;
						case "2":
							$(".L2").attr("selected", true);
							break;
						case "3":
							$(".L3").attr("selected", true);
							break;
						case "4":
							$(".L4").attr("selected", true);
							break;
						case "5":
							$(".L5").attr("selected", true);
							break;
						default:
							$(".L0").attr("selected", true);
						}
						...
						if(CU == "N"){
							$("#detail_ClassicUser").text("否");
						}else{
							$("#detail_ClassicUser").text("是");
						}
						//可编辑赋值
						...
						$user.getCommonCodes(b_class_cd);*/
					}else{
						var txt = "未查到查询用户详情！";
						window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
					}
