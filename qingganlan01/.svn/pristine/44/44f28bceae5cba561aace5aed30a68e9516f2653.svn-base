...
$(function() {
	...
	$(document).ready(function() {
		var columns = [{
				...
				field : 'FS_ALBUM_IMG_NO',
				title : '团契相册数量',
				sortable : true,
				width : 80
			},{
				field : 'FS_QUESITION_NO',
				title : '团契问题答疑数量',
				sortable : true,
				width : 80
			},{
			...
			options = {
				url : contextPath + '/mFSAction/getAllFSs.action',
				idField : 'FS_ID',
		...
		//创建easyUI datagrid
		$("#datagridFS").datagrid(options);
		$("#aj-basicSearch").click(function() {//搜索
			$("#datagridFS").datagrid("load",$.serializeObject($('#aj-searchFormnode')));
		});
		
		$("#microbasic-searchReset-btn").click(function() {
			$("#datagridFS").datagrid("load",{ });
			$('#uid').combobox('clear');
