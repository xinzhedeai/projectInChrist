...
$(document).ready(function() {
	$(document).ready(function() {
		...
		$makeFri.pageA = 1,rows = 12;//pageA指的是请求后台数据分页的page
		var flag = null;
	
		page=1;//初始化当前的版面为1
		$show=$("#zzsc").find(".zzsc_box");//找到图片展示区域
		
		getMakeFriPages($makeFri.pageA,rows);//初始页面请求接口
		
		$(".next").click(function(){
			$makeFri.pageA += 1;
			$makeFri.numsA = $makeFri.pageA * rows;
			if($makeFri.flagA == 1){//当设置全局标志位为数字时，初始值尽量不要是null，最好是1还是其他数字，否则在做判断是会有问题。
				getMakeFriPages($makeFri.pageA,rows);
			}
			//首先判断展示区域是否处于动画
		//	if(!$show.is(":animated")){
				$show.animate({left:'-='+$width_box},"normal");
				page++;
				nav(page,$makeFri.page_count);
				$number=page-1;
	...
	function getMakeFriPages(pageA,rows){//获取主内工作网站
		$.post('/userAction/getJobPages.action', $.param({'page':pageA,'rows':rows,'webType':'A05'}), function(res) {
			var rows = null;
			if (res.success) {
				rows = res.result.rows ;
				if(rows){
					$makeFri.total = res.result.total;
					var pageNum = Math.ceil($makeFri.total/12);
					...
					var p = '';//中文p
						...
						$(".zzsc_box").append(p);
						if($makeFri.numsA > $makeFri.total){
							$makeFri.flagA = 0;
						}
						$makeFri.page_count = pageNum;
						nav(page,$makeFri.page_count );
	...
	$.post('/userAction/addInChristWeb.action',$.param({'usId':loginUsId}) + '&' + $(".InChristWebForm").serialize(), function(result) {
		if (result.success) {
			var txt= "添加主内网址成功！";
			window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
			location.href = '/html/InChristMakeFri.html';
		} else {
			var txt= "添加主内网址失败！";
			window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
		}
